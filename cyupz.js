...
const { default: WAyuuection, BufferJSON, WA_DEFAULT_EPHEMERAL, generateWAMessageFromContent, proto, getBinaryNodeChildren, useMultiFileAuthState, generateWAMessageContent, downloadContentFromMessage, generateWAMessage, prepareWAMessageMedia, areJidsSameUser, getContentType } = require('@whiskeysockets/baileys')
...
module.exports = yuu = async (yuu, m, chatUpdate, store) => {
	try {
await LoadDataBase(yuu, m)
const botNumber = await yuu.decodeJid(yuu.user.id)
...
const qpayment = {key: {remoteJid: '0@s.whatsapp.net', fromMe: false, id: `ownername`, participant: '0@s.whatsapp.net'}, message: {requestPaymentMessage: {currencyCodeIso4217: "USD", amount1000: 999999999, requestFrom: '0@s.whatsapp.net', noteMessage: { extendedTextMessage: { text: "Turzz Botz"}}, expiryTimestamp: 999999999, amount: {value: 91929291929, offset: 1000, currencyCode: "USD"}}}}

const qtoko = {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(m.chat ? {remoteJid: "status@broadcast"} : {})}, message: {"productMessage": {"product": {"productImage": {"mimetype": "image/jpeg", "jpegThumbnail": ""}, "title": `${namaOwner} - Marketplace`, "description": null, "currencyCode": "IDR", "priceAmount1000": "9999999999999", "retailerId": `Powered By ${namaOwner}`, "productImageCount": 1}, "businessOwnerJid": `0@s.whatsapp.net`}}}
...
return yuu.sendMessage(m.chat, {text: `
...
var gclink = (`https://chat.whatsapp.com/` + await yuu.groupInviteCode(m.chat))
var isLinkThisGc = new RegExp(gclink, 'i')
var isgclink = isLinkThisGc.test(m.text)
if (isgclink) return
let delet = m.key.participant
let bang = m.key.id
await yuu.sendMessage(m.chat, {text: `*Link Grup Terdeteksi*
...
await yuu.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
await sleep(1000)
await yuu.groupParticipantsUpdate(m.chat, [m.sender], "remove")
}}
...
await yuu.sendMessage(m.chat, {text: `
...
yuu.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
  let type = await yuu.getFile(path, true);
  ...
  try {
    m = await yuu.sendMessage(jid, message, { ...opt, ...options });
  } catch (e) {
    //console.error(e)
    m = null;
  } finally {
    if (!m) m = await yuu.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options });
    file = null;
    return m;
  }
}
...
return yuu.sendMessage(m.chat, {text: teks, mentions: [m.sender], contextInfo: {
externalAdReply: {
title: botname, 
...
let imgsc = await prepareWAMessageMedia({ image: { url: global.image.logo }}, { upload: yuu.waUploadToServer })
...
text: "*All Transaksi Open ✅*\n\n*Turzz Hosting* Menyediakan Produk & Jasa Dibawah Ini ⬇️"
}), 
contextInfo: {
...
await yuu.relayMessage(jid, msgii.message, {messageId: msgii.key.id})
}
...
case "menu": {
yuu.sendMessage(m.chat, { react: { text: `🧭`, key: m.key }})
let teksnya = `Haii @${m.sender.split("@")[0]},
╭─────═[ DASBOARD ]═─────• 
┊╭───────────────··· 
┴┊ 
⬡┣ *❁* *NAMA BOT :* *${global.botname2}*
⬡┣ *❁* *PENGEMBANG:* @${global.owner}
⬡┣ *❁* *VERSIONS :* *${global.versi}*
⬡┣ *❁* *TYPE SCRIPT :* *Case F* 
⬡┣ *❁* *STATUS KAMU :* *(${isCreator ? "Ownerbot" : isPremium ? "Reseller Panel" : "Vip User"})*
⬡┣ *❁* *MODE :* *${yuu.public ? "Public": "privat"}*
┬┊
┊╰───────────────··· 
╰─────═══════════─────•`

await yuu.sendMessage(m.chat, {
  footer: `since 2020 by ${botname}`,
  buttons: [
    {
      buttonId: `.allmenu`,
...
highlight_label: 'Populerr',
rows: [
  {
    header: '',
    title: 'Balik Ke menu',
    description: 'ke all menu',
    id: '.allmenu',
  },
  {
    header: '',
...
if (m.quoted.fromMe) {
yuu.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: true, id: m.quoted.id, participant: m.quoted.sender}})
} else {
if (!m.isBotAdmin) return Reply(mess.botAdmin)
yuu.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: m.quoted.id, participant: m.quoted.sender}})
}} else {
...
await yuu.updateBlockStatus(mem, "unblock");
if (m.isGroup) yuu.sendMessage(m.chat, {text: `Berhasil membuka blokiran @${mem.split('@')[0]}`, mentions: [mem]}, {quoted: m})
}
break
...
const allgrup = await yuu.groupFetchAllParticipating()
const res = await Object.keys(allgrup)
let count = 0
const teks = text
const jid = m.chat
const rest = await yuu.downloadAndSaveMediaMessage(qmsg)
await m.reply(`Memproses jpm testimoni ke dalam channel & ${res.length} grup`)
await yuu.sendMessage(global.idSaluran, {image: await fs.readFileSync(rest), caption: teks})
for (let i of res) {
if (global.db.groups[i] && global.db.groups[i].blacklistjpm && global.db.groups[i].blacklistjpm == true) continue
try {
await yuu.sendMessage(i, {
  footer: `© 2025 ${botname}`,
  buttons: [
    {
    buttonId: 'action',
...
await yuu.sendMessage(jid, {text: `Testimoni berhasil dikirim ke dalam channel & ${count} grup`}, {quoted: m})
}
break
...
await yuu.sendMessage(m.chat, {react: {text: '🔎', key: m.key}})
...
await yuu.sendMessage(m.chat, {audio: {url: urlMp3}, mimetype: "audio/mpeg", contextInfo: { externalAdReply: {thumbnailUrl: res.thumbnail, title: res.title, body: `Author ${res.author.name} || Duration ${res.timestamp}`, sourceUrl: res.url, renderLargerThumbnail: true, mediaType: 1}}}, {quoted: m})
} else {
...
await yuu.sendMessage(m.chat, {video: {url: urlMp3}, ptv: true, mimetype: "video/mp4"}, {quoted: m})
} else {
return m.reply("Error! vidio atau lagu tidak ditemukan")
}
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
...
if (anu.download.audio) {
let urlMp3 = anu.download.audio
await yuu.sendMessage(m.chat, {audio: {url: urlMp3}, mimetype: "audio/mpeg"}, {quoted: m})
} else {
return m.reply("Error! vidio atau lagu tidak ditemukan")
}
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
...
if (m.isGroup && !m.quoted && !text) return m.reply(example("@tag/nomornya"))
const mem = !m.isGroup ? m.chat : m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text ? text.replace(/[^0-9]/g, "") + "@s.whatsapp.net" : ""
await yuu.updateBlockStatus(mem, "block")
if (m.isGroup) yuu.sendMessage(m.chat, {text: `Berhasil memblokir @${mem.split('@')[0]}`, mentions: [mem]}, {quoted: m})
}
break
...
if (anu.download.video) {
let urlMp3 = anu.download.video
await yuu.sendMessage(m.chat, {video: {url: urlMp3}, mimetype: "video/mp4"}, {quoted: m})
} else {
return m.reply("Error! vidio atau lagu tidak ditemukan")
}
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
...
await yuu.sendMessage(m.chat, {document: {url: res.link}, fileName: res.judul, mimetype: "application/"+res.mime.toLowerCase()}, {quoted: m})
...
await yuu.sendMessage(m.chat, {audio: {url: res.music_info.url}, mimetype: "audio/mpeg"}, {quoted: m})
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
}).catch((e) => m.reply("Error"))
}
break
...
if (!res.status) return m.reply("Error! Result Not Found")
await yuu.sendMessage(m.chat, {video: {url: res.result.url}, mimetype: "video/mp4", caption: "*video Instagram berhasil ke download ✅ ✅*"}, {quoted: m})
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
}).catch((e) => m.reply("Error"))
}
break
...
if (!res.status) return m.reply("Error! Result Not Found")
await yuu.sendMessage(m.chat, {video: {url: res.result.sd}, mimetype: "video/mp4", caption: "*video Facebook berhasil ke download ✅ ✅*"}, {quoted: m})
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
}).catch((e) => m.reply("Error"))
}
break
...
if (!res.status) return m.reply("Error! Result Not Found")
await yuu.sendMessage(m.chat, {video: {url: res.result.video}, mimetype: "video/mp4", caption: "*video Capcut berhasil ke download ✅ ✅*"}, {quoted: m})
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
}).catch((e) => m.reply("Error"))
}
break
...
try {
    ...
    yuu.sendMessage(m.chat, { document: { url: url }, mimetype: 'application/zip', fileName: `${filename}`}, { quoted : m })
} catch (e) {
...
let imgsc = await prepareWAMessageMedia({ image: {url: `${a.url}`}}, { upload: yuu.waUploadToServer })
await araara.push({
header: proto.Message.InteractiveMessage.Header.fromObject({
...
await yuu.relayMessage(m.chat, msgii.message, { 
messageId: msgii.key.id 
})
} else {
let urlVid = await result.data.find(e => e.type == "nowatermark_hd" || e.type == "nowatermark")
await yuu.sendMessage(m.chat, {video: {url: urlVid.url}, mimetype: 'video/mp4', caption: `*video tiktok berhasil ke download ✅*`}, {quoted: m})
}
...
await yuu.sendMessage(m.chat, { image: data, mimetype: "image/png"}, {quoted: m})
}
break
...
await JsConfuser.obfuscate(await fs.readFileSync(`./database/sampah/${filename}`).toString(), {
  ...
  await yuu.sendMessage(m.chat, {document: fs.readFileSync(`./database/sampah/${filename}`), mimetype: "application/javascript", fileName: filename, caption: "Encrypt file sukses ✅"}, {quoted: m})
...
await JsConfuser.obfuscate(await fs.readFileSync(`./@hardenc${filename}.js`).toString(), {
  ...
  await yuu.sendMessage(m.chat, {document: fs.readFileSync(`./@hardenc${filename}.js`), mimetype: "application/javascript", fileName: filename, caption: "Encrypt File JS Sukses! Type:\nString"}, {quoted: m})
}).catch(e => m.reply("Error :" + e))
...
await yuu.sendMessage(m.chat, {text: a.url}, {quoted: m})
}
break
...
if (!isCreator) return
let teks = `\n *List all group chat*\n`
let a = await yuu.groupFetchAllParticipating()
...
let res = await yuu.newsletterMetadata("invite", result)
let teks = `
* *ID :* ${res.id}
* *Nama :* ${res.name}
...
let urutan = 0
for (let a of pin) {
let imgsc = await prepareWAMessageMedia({ image: {url: `${a.images_url}`}}, { upload: yuu.waUploadToServer })
await araara.push({
header: proto.Message.InteractiveMessage.Header.fromObject({
hasMediaAttachment: true,
...imgsc
...
for (let i of aray) {
await yuu.sendMessage(m.chat, {image: {url: i.url}, caption: `Hasil pencarian foto ke ${total += 1}`}, {quoted: m})
}
await yuu.sendMessage(m.chat, {react: {text: '', key: m.key}})
}
break
...
try {
await yuu.sendAsSticker(m.chat, videoBuffer, m, {packname: global.packname})
} catch {}
}
break
...
let response = await axios.get(brat, { responseType: "arraybuffer" });
let videoBuffer = response.data;
let stickerBuffer = await yuu.sendAsSticker(m.chat, videoBuffer, m, {
...
var ppuser
try {
ppuser = await yuu.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
await yuu.sendAsSticker(m.chat, tempnya, m, {packname: global.packname})
...
var image = await yuu.downloadAndSaveMediaMessage(qmsg)
await yuu.sendAsSticker(m.chat, image, m, {packname: global.packname})
...
if (/video/gi.test(mime) && qmsg.seconds > 15) return m.reply("Durasi vidio maksimal 15 detik!")
var image = await yuu.downloadAndSaveMediaMessage(qmsg)
await yuu.sendAsSticker(m.chat, image, m, {packname: text})
await fs.unlinkSync(image)
}
break
...
let media = await downloadContentFromMessage(msg[type], type == 'imageMessage' ? 'image' : type == 'videoMessage' ? 'video' : 'audio')
    ...
    if (/video/.test(type)) {
        return yuu.sendMessage(m.chat, {video: buffer, caption: msg[type].caption || ""}, {quoted: m})
    } else if (/image/.test(type)) {
        return yuu.sendMessage(m.chat, {image: buffer, caption: msg[type].caption || ""}, {quoted: m})
    } else if (/audio/.test(type)) {
        return yuu.sendMessage(m.chat, {audio: buffer, mimetype: "audio/mpeg", ptt: true}, {quoted: m})
    } 
}
break
...
yuu.sendFile(m.chat, enhanced, "", "_Sukses Membuat HD 3x Enhance✅_", m);
} catch (err) {
console.error(err);
await yuu.sendMessage(m.chat, {react: {text: '❌', key: m.key}})
}
...
var onWa = await yuu.onWhatsApp(input.split("@")[0])
if (onWa.length < 1) return m.reply("Nomor tidak terdaftar di whatsapp")
const res = await yuu.groupParticipantsUpdate(m.chat, [input], 'add')
...
var onWa = await yuu.onWhatsApp(input.split("@")[0])
if (onWa.length < 1) return m.reply("Nomor tidak terdaftar di whatsapp")
const res = await yuu.groupParticipantsUpdate(m.chat, [input], 'remove')
...
if (!m.isGroup) return Reply(mess.group)
if (!m.isBotAdmin) return Reply(mess.botAdmin)
await yuu.groupRevokeInvite(m.chat)
...
await yuu.sendMessage(m.chat, {text: teks, mentions: [...member]}, {quoted: m})
}
break
...
if (!m.isGroup) return Reply(mess.group)
if (!m.isBotAdmin) return Reply(mess.botAdmin)
const urlGrup = "https://chat.whatsapp.com/" + await yuu.groupInviteCode(m.chat)
var teks = `
${urlGrup}
`
await yuu.sendMessage(m.chat, {text: teks, matchedText: `${urlGrup}`}, {quoted: m})
}
break
...
await yuu.sendMessage(m.chat, {text: text, mentions: [...member]}, {quoted: m})
}
break
...
let id = await yuu.groupAcceptInvite(result)
...
let res = await yuu.newsletterMetadata("invite", result)
await yuu.newsletterFollow(res.id)
m.reply(`
...
await yuu.groupSettingUpdate(m.chat, 'not_announcement')
} else if (/closegc|close/.test(command)) {
if (m.metadata.announce == true) return 
await yuu.groupSettingUpdate(m.chat, 'announcement')
} else {}
}
break
...
await yuu.groupParticipantsUpdate(m.chat, [i], 'remove')
await sleep(1000)
}
...
await yuu.groupParticipantsUpdate(m.chat, [target], action.toLowerCase()).then(async () => {
await yuu.sendMessage(m.chat, {text: `Sukses ${action.toLowerCase()} @${target.split("@")[0]}`, mentions: [target]}, {quoted: m})
})
} else {
...
console.log('yuuection Error: ' + err);
...
await yuu.sendMessage(orang, {document: fs.readFileSync("./akunpanel.txt"), fileName: "akunpanel.txt", mimetype: "text/plain", caption: teks}, {quoted: m})
...
yuu.sendMessage(m.chat, {text: teks, mentions: premium}, {quoted: m})
}
break
...
if (!text) return yuu.sendMessage(m.chat, {
  buttons: [
    {
    buttonId: 'action',
    buttonText: { displayText: 'ini pesan interactiveMeta' },
...
`
let msgQr = await yuu.sendMessage(m.chat, {
  footer: `© 2025 ${botname}`,
  buttons: [
    {
      buttonId: `.batalbeli`,
...
await yuu.sendMessage(db.users[m.sender].saweria.chat, {text: "QRIS Pembayaran telah expired!"}, {quoted: db.users[m.sender].saweria.msg})
await yuu.sendMessage(db.users[m.sender].saweria.chat, { delete: db.users[m.sender].saweria.msg.key })
...
await yuu.sendMessage(db.users[m.sender].saweria.chat, {text: `
...
await yuu.sendMessage(orang, {document: await fs.readFileSync(Obj.file), mimetype: "application/zip", fileName: Obj.namaSc}, {quoted: null})
await yuu.sendMessage(db.users[m.sender].saweria.chat, { delete: db.users[m.sender].saweria.msg.key })
delete db.users[m.sender].saweria
}
}
...
chpasswd: { expire: False }`;
        ...
        if (response.ok) {
            ...
            await yuu.sendMessage(orang, { text: messageText });
        } else {
...
await yuu.sendMessage(orang, {document: fs.readFileSync("./akunpanel.txt"), fileName: "akunpanel.txt", mimetype: "text/plain", caption: tekspanel}, {quoted: null})
await fs.unlinkSync("./akunpanel.txt")
await yuu.sendMessage(db.users[m.sender].saweria.chat, { delete: db.users[m.sender].saweria.msg.key })
delete db.users[m.sender].saweria
}
...
`
await fs.writeFileSync("./akunpanel.txt", teks)
await yuu.sendMessage(db.users[m.sender].saweria.chat, {document: fs.readFileSync("./akunpanel.txt"), fileName: "akunpanel.txt", mimetype: "text/plain", caption: teks}, {quoted: m})
await fs.unlinkSync("./akunpanel.txt")
await yuu.sendMessage(db.users[m.sender].saweria.chat, { delete: db.users[m.sender].saweria.msg.key })
delete db.users[m.sender].saweria
}
...
await yuu.sendMessage(m.chat, {text: "Berhasil membatalkan pembelian ✅"}, {quoted: db.users[m.sender].saweria.msg})
await yuu.sendMessage(m.chat, { delete: db.users[m.sender].saweria.msg.key })
await clearInterval(db.users[m.sender].saweria.exp)
delete db.users[m.sender].saweria
} else {
...
yuu.sendMessage(m.chat, { text: mesej }, {quoted: m});
});
} catch (err) {
...
await sleep(60000) 
yuu.sendMessage(m.chat, { text: textvps }, {quoted: m});
} else {
...
chpasswd: { expire: False }`;
        ...
        if (response.ok) {
            ...
            await yuu.sendMessage(m.chat, { text: messageText });
        } else {
            throw new Error(`Gagal membuat VPS: ${responseData.message}`);
        }
    } catch (err) {
...
let res = await yuu.groupMetadata(text)
...
await yuu.sendMessage(m.sender, { document: fs.readFileSync("./library/database/contacts.vcf"), fileName: "contacts.vcf", caption: `File kontak berhasil dibuat ✅\nTotal *${halls.length}* kontak`, mimetype: "text/vcard", }, { quoted: m })
...
if (!isOwner) return Reply(mess.owner)
if (!text) return m.reply(example("pesannya"))
const meta = await yuu.groupFetchAllParticipating()
...
const data = await yuu.groupMetadata(idgc)
const halls = await data.participants.filter(v => v.id.endsWith('.net')).map(v => v.id)
await m.reply(`Memproses *pushkontak* ke dalam grup *${data.subject}*`)

for (let mem of halls) {
if (mem !== botNumber && mem.split("@")[0] !== global.owner) {
await yuu.sendMessage(mem, {text: teks}, {quoted: qlocPush })
await sleep(global.delayPushkontak)
}}

delete global.textpushkontak
await yuu.sendMessage(jidawal, {text: `*Berhasil Pushkontak ✅*\nTotal member berhasil dikirim pesan : ${halls.length}`}, {quoted: m})
}
break
...
const teks = text
const jidawal = m.chat
const data = await yuu.groupMetadata(m.chat)
const halls = await data.participants.filter(v => v.id.endsWith('.net')).map(v => v.id)
...
case "jpmslide": {
if (!isCreator) return Reply(mess.owner)
let allgrup = await yuu.groupFetchAllParticipating()
...
await yuu.sendMessage(jid, {text: `*Jpm Telah Selsai ✅*\nTotal grup yang berhasil dikirim pesan : ${count}`}, {quoted: m})
}
break
...
if (global.db.groups[i] && global.db.groups[i].blacklistjpm && global.db.groups[i].blacklistjpm == true) continue
try {
await yuu.sendMessage(i, {text: `${teks}`}, {quoted: qlocJpm})
count += 1
} catch {}
await sleep(global.delayJpm)
}
...
if (global.db.groups[i] && global.db.groups[i].blacklistjpm && global.db.groups[i].blacklistjpm == true) continue
try {
await yuu.sendMessage(i, {image: fs.readFileSync(rest), caption: teks}, {quoted: qlocJpm})
count += 1
} catch {}
await sleep(global.delayJpm)
}
...
`
await yuu.sendMessage(m.chat, {text: teks}, {quoted: qtext2})
}
break
...
const vid = await yuu.downloadAndSaveMediaMessage(qmsg)
const result = await toAudio(fs.readFileSync(vid), "mp4")
await yuu.sendMessage(m.chat, { audio: result, mimetype: "audio/mpeg", ptt: /tovn/.test(command) ? true : false }, { quoted: m })
...
*Marketplace :*
${linkGrup}`
await yuu.sendMessage(m.chat, {text: teks, mentions: [m.sender], contextInfo: {
externalAdReply: {
title: `Dana Masuk ✅`, 
body: `© Powered By ${namaOwner}`, 
thumbnailUrl: global.image.reply, 
...
await yuu.sendContact(m.chat, [global.owner], m)
}
break
...
case "save": case "sv": {
if (!isCreator) return
await yuu.sendContact(m.chat, [m.chat.split("@")[0]], m)
}
break
...
const getcase = (cases) => {
const fileContent = fs.readFileSync('./cyupz.js').toString();
...
if (!isCreator) return Reply(mess.owner)
if (!text) return m.reply(example("teksnya"))
await yuu.sendMessage(idSaluran, {text: text})
...
let img = await yuu.downloadAndSaveMediaMessage(qmsg)
await yuu.sendMessage(idSaluran, {image: await fs.readFileSync(img), caption: text})
...
await yuu.sendMessage(m.sender, {document: await fs.readFileSync(`./${name}.zip`), fileName: `${name}.zip`, mimetype: "application/zip"}, {quoted: m})
...
var medis = await yuu.downloadAndSaveMediaMessage(qmsg)
...
await yuu.updateProfilePicture(botNumber, {content: medis})
await fs.unlinkSync(medis)
m.reply("Berhasil mengganti foto profil bot ✅")
}
...
if (!isCreator) return Reply(mess.owner)
yuu.chatModify({ delete: true, lastMessages: [{ key: m.key, messageTimestamp: m.timestamp }]}, m.chat)
}
break
...
yuu.sendMessage(m.chat, {text: teks, mentions: owners}, {quoted: m})
}
break
...
yuu.sendMessage(Obj + "@s.whatsapp.net", {text: `*Hallo developer, telah terjadi error pada command :* ${isCmd ? prefix+command : m.text}
